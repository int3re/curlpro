...
## Известное ограничение: QPACK

Профиль объявляет `QPACK_MAX_TABLE_CAPACITY: 65536`, как Chrome, но
вендоренный декодер динамическую таблицу не поддерживает (ограничение
апстрима: «Our QPACK implementation doesn't use the dynamic table yet»).

Сервер, который решит ей воспользоваться, вернёт ответ, который мы не разберём:
`qpack: expected Required Insert Count to be zero`. Наблюдалось на
`fp.impersonate.pro` со второго запроса; `quic.browserleaks.com` таблицу
не использует и работает стабильно.

Выбор между «совпадающим отпечатком» и «работающим декодером» сделан в пользу
отпечатка: занизить объявленную ёмкость значит разойтись с Chrome в первом же
поле SETTINGS. Закрывается реализацией динамической таблицы QPACK.
