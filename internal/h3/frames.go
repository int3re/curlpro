...
func (f *settingsFrame) settingsMap() map[uint64]uint64 {
	...
	if f.MaxFieldSectionSize >= 0 {
		out[settingMaxFieldSectionSize] = uint64(f.MaxFieldSectionSize)
	}
	if f.Datagram {
		out[settingDatagram] = 1
	}
	if f.ExtendedConnect {
		out[settingExtendedConnect] = 1
	}
	return out
}
...
func (f *settingsFrame) settingsSequence(all map[uint64]uint64) []uint64 {
	...
	rest := make([]uint64, 0, len(all)-len(seq))
	for id := range all {
		if !seen[id] {
			rest = append(rest, id)
		}
...
func (f *settingsFrame) Append(b []byte) []byte {
	all := f.settingsMap()
	seq := f.settingsSequence(all)

	b = quicvarint.Append(b, 0x4)
	var l int
	for _, id := range seq {
		l += quicvarint.Len(id) + quicvarint.Len(all[id])
	}
	b = quicvarint.Append(b, uint64(l))
	for _, id := range seq {
		b = quicvarint.Append(b, id)
		b = quicvarint.Append(b, all[id])
	}
	return b
}
