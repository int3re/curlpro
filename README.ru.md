...
[Навигация и fetch](#навигация-и-fetch) · [Загрузка страницы](#загрузка-страницы-и-ресурсы) · [Профили как данные](#профили-как-данные) ·
...
- **Нативная асинхронность**: запрос уходит в горутину, поток на процесс — один;
  на free-threaded Python (3.14t) GIL остаётся выключенным, и потоки разбирают параллельно.
- **Загрузка страницы, как у браузера**: `load_page()` берёт документ и стили,
  скрипты, картинки, шрифты и фреймы, которые он называет, каждый своим видом
  ресурса — со своим `Accept`, `sec-fetch-dest`, `priority`, порядком заголовков.
- **Соединения, как у браузера**: пачка запросов гоняет столько рукопожатий,
  сколько браузер, и едет по одному HTTP/2-соединению; Chromium объединяет
  имена по адресу и разводит запросы с учётными данными и без.
...
| Заголовки | набор навигации, набор `fetch` и набор на каждый вид ресурса, позиции слотов, якорь пользовательских |
| Соединения | рукопожатия на пачку, HTTP/2 на несколько имён, разделение по privacy mode и по сайту |
...
| `connect_timeout`, `response_timeout` | те же пределы по имени: установка соединения (резолв, TCP, TLS) и ожидание заголовков ответа. `response_timeout` закрывает дыру между двумя другими — сервер принял соединение и думает — и тело не ограничивает: как только заголовки пришли, действует только `timeout` |
...
| `max_response_size` | предел размера тела; бесконечный ответ съел бы память. Не задан: 100 МиБ для буферизованного ответа, без предела для потока; `0` — без предела. Ограничивает `read()`, но не `iter_content()` |
...
| `headers`, `header_order` | свои заголовки и порядок отправки как шаблон — `[..., "accept", "x-api-key", ...]` ставит заголовок сразу после Accept, `...` — собственный порядок профиля (см. ниже); значение `None` убирает заголовок, который послали бы профиль или сессия, `""` шлёт его пустым |
| `protocol` | `1.1`/`http1`, `2`/`h2`, `3`/`h3` — транспорт этого запроса |
| `timeout` | число или пара `(соединение, всего)` |
| `connect_timeout`, `response_timeout` | предел на соединение и на ожидание заголовков по имени; `connect_timeout` сильнее первого элемента пары |
...
s.get(url, headers={"Sec-Fetch-User": None})   # без одного заголовка профиля, остальные на месте
```
...
Заголовок со значением `None` убирается — из набора профиля или сессии, — а
остальное уходит ровно так, как шлёт браузер, вместе с порядком. Так теряют один
навигационный заголовок без `default_headers=False`, который теряет и User-Agent,
и порядок; на сессии то же для каждого запроса делает `s.headers["Sec-Fetch-User"]
= None`, а `s.headers.suppressed` перечисляет, чего нет. Пустая строка — значение:
она уходит пустым заголовком, как шлёт его браузерный `fetch()`.

## Порядок заголовков

Порядок — часть отпечатка, поэтому кастомный заголовок без просьбы встаёт туда,
куда его поставил бы браузер: перед якорем профиля. Когда нужно иначе,
`header_order` — шаблон поверх порядка браузера: имена и `...` в значении
«здесь собственные заголовки профиля».
...
`...` может быть несколько: не названный заголовок профиля остаётся рядом с
названным соседом, за которым идёт в профиле. Список без `...` — это список,
а за ним остаток профиля. Имена, которых в запросе нет, пропускаются, так что
один шаблон на сессии подходит всем запросам; имя дважды отклоняется.
`fingerprint().headers` показывает результат до отправки.
...
| `encoding` | кодировка тела — из `Content-Type`, BOM, затем документа — именно эта; `cp1251` и `windows-1251` равны. Ловит страницу, пришедшую в cp1251 там, где код ждал UTF-8: `.text` молча превратил бы её в кракозябры |
...
С 0.12 откат запроса точен: нативная часть записывает, какие куки запрос
изменил, и откатывает только их, так что куки, полученные тем временем запросом
другого потока, остаются. `transaction()` восстанавливает снимок, сделанный до
блока. Половина логина хуже, чем его отсутствие.
...
with curlpro.Session("chrome-152-android", device="random") as s:
    s.get(url)          # User-Agent: Android 15; SM-S911B — и sec-ch-ua-model:
                        # "SM-S911B", как только сайт запросил подсказки
```

Устройство выбирается один раз на сессию: настоящий клиент телефон между запросами
не меняет. В пуле 46 настоящих телефонов — точные строки `ro.product.model` из
каталога устройств Google Play, с уклоном в российский рынок, каждый на
правдоподобной версии Android, — так что `device="random"` выбирает достоверный
телефон при одном и том же TLS. Свой список задаётся параметром `devices`.

Одно намеренное отступление от Chrome, решение владельца проекта: выбранное
устройство пишется и в `User-Agent` (`Android 15; SM-S911B`), как это делает
Яндекс Браузер, так что 46 телефонов — это 46 строк и у Chrome. Стоковый Chrome
≥110 шлёт сокращённое `Android 10; K` и раскрывает модель только в подсказках,
...
Считаются не только имена, но и значения: запрос с `sec-fetch-mode: cors` или с
`sec-fetch-dest`, которого у навигации не бывает, — это fetch, что бы он ещё ни
нёс. Явный `mode="fetch"` на профиле без набора fetch — Safari, okhttp —
...
## Загрузка страницы и ресурсы

Страница — это не только документ: браузер запрашивает каждый стиль, скрипт,
картинку, шрифт и фрейм, которые называет разметка, каждый своим видом ресурса,
а антибот смотрит, как это происходит. `resource=` шлёт запрос одним из этих
видов, а `load_page()` грузит документ с ресурсами так, как это делает браузер, —
разом, в порядке разметки, иконку последней:

```python
page = s.load_page("https://example.com/", css=True)  # css: шрифты, объявленные стилями
for r in page.resources:
    print(r.kind, r.status, r.url)                    # style 200 …, script-async 200 …, image 200 …

s.get(img_url, resource="image", page=page.url)       # один ресурс вручную
s.get(font_url, resource="font", page=page.url)       # шрифт: CORS, Origin, свой Accept
```

Семнадцать видов — `style`, `script`, `script-async`, `module`, `image`, `icon`,
`font`, `iframe`, `prefetch`, `beacon` и остальные — сняты на Chrome 153 и
Firefox 156 по HTTP/2 и HTTP/1.1, вплоть до `u=1` у скрипта в `<head>` против
`u=2` у скрипта в `<body>` и `Accept-Encoding: identity`, с которым Firefox
просит шрифт стиля (`docs/STAGE21-RESULTS.md`). Пул тратит соединения так, как
семейство: пачка первых запросов гоняет четыре рукопожатия у Chromium и шесть у
Firefox и едет по первому HTTP/2-соединению; Chromium отдаёт HTTP/2-соединение
другому имени на том же адресе, если его покрывает сертификат, и держит запросы
без учётных данных и запросы со страниц других сайтов на своих соединениях.
