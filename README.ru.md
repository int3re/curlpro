...
- **48 профилей**: Chrome 98–152, Edge, Firefox 133–155, Safari, Tor, Яндекс.Браузер;
...
Ни Go, ни компилятора не нужно: нативная библиотека и все 48 профилей уже
...
| `proxy` | `http://`, `https://`, `socks5://` или `socks5h://`, можно с `user:pass`. Адрес без схемы — `1.2.3.4:8080` — читается как `http://`; определения на ощупь нет, SOCKS надо назвать явно. `HTTP_PROXY` читается для запросов по `http://`, `HTTPS_PROXY` — по `https://`, `ALL_PROXY` для обоих, `NO_PROXY` исключает |
...
зависела от чужого сервиса. Расчёт сверен с 48 захватами из
`reference/baselines`: **JA4 48/48, JA3N 48/48, Akamai 48/48**.
