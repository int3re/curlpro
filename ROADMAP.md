...
| A cross-origin fetch sends no CORS preflight | measured on Chrome 153 and Firefox 156 (STAGE17): a fetch with a JSON content type or a custom header is preceded by an `OPTIONS` with a header set of its own. The library sends the request straight out, so a server that logs preflights sees a request no browser makes. Wanted alongside it: withholding cookies on an uncredentialed cross-origin fetch |
